...
@AndroidEntryPoint
class MoviesHomeScreenFragment : Fragment() {

    private var _binding: FragmentMoviesHomeScreenBinding? = null
    private val binding get() = _binding!!

    private var starCastAdapter: StarCastAdapter? = null
    private var moviesRelatedAdapter: MoviesRelatedAdapter? = null

    private val args: MoviesHomeScreenFragmentArgs by navArgs()

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?,
    ): View? {
        _binding = FragmentMoviesHomeScreenBinding.inflate(layoutInflater, container, false)
        return binding.root
    }
    ...
    private fun setBackgroundImage() {
        with(binding) {
            spBgImg.setImageResource(args.MovieImageItem.img)
